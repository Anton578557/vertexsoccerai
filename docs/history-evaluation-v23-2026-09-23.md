...
Application commit `2a66d5d` was deployed Ready. Coventry–Brighton now uses BSD's 8+8 form matches; Football-Data recovered Brighton's verified country and crest. Both images loaded. Wehen–Aachen remains 8+8 and the obsolete 1/1 warning is gone. Repeating it preserved the original four market snapshots byte-for-byte and kept their original creation times. News checks on Porto removed the unrelated opponent-injury and scouting articles; Rayo's old previews and Valverde injury article were excluded.

The actual **Run** action in Vercel's enabled Cron Jobs panel invoked the protected endpoint successfully at **2026-09-23 21:09:45 UTC**. It checked the one unresolved legacy fixture again, recorded its pending status and left settled results unchanged. Requests imitating the cron user agent without its secret return 401. Thus both the platform invocation and the maintenance handler were verified, rather than relying only on a successful public summary response.

All **81 automated tests** pass. The full repository quality workflow passes locally: all JavaScript syntax, model tests, configuration, secret scan, model integrity, API access/rate limits and runtime guards. Its previously stale model-version check was replaced by a valid exported-version check. The secret scanner now covers BSD/cron keys too and exempts only four exact inert values in test files; actual credentials are still rejected there.
