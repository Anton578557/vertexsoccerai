...
const { enrichFootballData, recoverFootballDataMetadata } = require('./football-data-enrichment');
const { buildOpenHistoricalContext } = require('./football-data-uk');
const { getNewsIntelligence } = require('./news-intelligence');
const { applyHistory } = require('./verified-history');
...
async function enhanceAnalysis(base) {
  ...
  if (fd.ok) {
    await recoverFootballDataMetadata(base, fd.teamIdentities);
    const fresh = side => fd.history?.[side]?.some(row => Date.now() - Date.parse(row.date) < 90 * 864e5);
    applyHistory(base, {...fd, source: 'Football-Data', ok: fd.homeForm?.played >= 3 && fd.awayForm?.played >= 3 && fresh('home') && fresh('away')});
    if (fd.fixture) {
      base.fixture = {
        ...base.fixture,
  ...
  applyHistory(base, {...openHistory, source: 'Football-Data.co.uk'});
  if(openHistory.ok && openHistory.leagueName && !base.fixture?.date && base.historySelection?.selected === 'Football-Data.co.uk') {
    base.fixture.league=openHistory.leagueName;
    base.fixture.leagueSource='Football-Data.co.uk verified results';
  }
  ...
  base.newsStatus = {status:newsIntel.status,checkedAt:newsIntel.checkedAt,providers:newsIntel.providers,partial:newsIntel.partial};
  base.sourceStatus.news = newsIntel.items.length ? newsIntel.providers.filter(p=>p.accepted>0).map(p=>p.provider).join(' + ') : newsIntel.status;
