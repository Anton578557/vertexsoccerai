...
function weatherContext(weather, fixtureDate) {
  ...
  const wind = weather.windMs == null ? NaN : Number(weather.windMs);
  const temp = weather.tempC == null ? NaN : Number(weather.tempC);
  const humidity = weather.humidity == null ? NaN : Number(weather.humidity);
...
module.exports = { enhanceAnalysis, weatherContext };
