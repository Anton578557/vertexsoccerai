...
async function checkAnalyzerCases() {
  const { payload } = await cachedProviderCall({
    cacheKey: 'analyzer-health:v1', provider: 'Vertex Diagnostics', ttlSeconds: 86400, staleSeconds: 86400,
    loader: async () => ({ timestamp: new Date().toISOString(), checks: await Promise.all(CASES.map(async ([home, away]) => {
      try {
        ...
        coverage: analysis.marketCoverage?.granular || null, availability: analysis.availability?.code,
