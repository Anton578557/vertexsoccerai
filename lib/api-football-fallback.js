...
function youthOrReserveName(value) {
  return /\b(youth|academy|reserve|reserves|u\s?-?\d{2}|under\s?-?\d{2}|primavera|b team|ii)\b/i.test(clean(value));
}

function wantsYouthOrReserve(value) {
...
async function resolveTeam(name) {
  // v2 avoids old cached short-name results such as "Milan" -> "Milan Youth".
  const key = `api-football:v2:team:${safeKey(name)}`;
