...
async function resolveTeam(name) {
  ...
  const rows = (Array.isArray(data?.response) ? data.response : []).filter(row => sameTeam(row?.team?.name, name));
...
function mapFixture(item) {
  return {
    ...
    homeScore: item?.fixture?.status?.short === 'FT' && Number.isInteger(item?.goals?.home) && item.goals.home >= 0 ? item.goals.home : null,
    awayScore: item?.fixture?.status?.short === 'FT' && Number.isInteger(item?.goals?.away) && item.goals.away >= 0 ? item.goals.away : null,
...
function formStats(events, teamName) {
  const rows = events.filter((m) => new Date(m.date).getTime() < Date.now() && new Date(m.date).getTime() > Date.now() - 180*864e5 && (sameTeam(m.home, teamName) || sameTeam(m.away, teamName)) && Number.isFinite(m.homeScore) && Number.isFinite(m.awayScore))
...
async function enrichApiFootballFallback(analysis) {
  ...
  if (paused) { analysis.sourceStatus.apiFootball = paused.reason; return {analysis,used:false,cacheHits:0}; }
  ...
  if (needFixture) {
    ...
    const match = next.rows.find(fixture => fixture.status === 'NS' && new Date(fixture.date).getTime() > Date.now() && ((sameTeam(fixture.home,homeResolved.team.name) && sameTeam(fixture.away,awayResolved.team.name)) || (sameTeam(fixture.home,awayResolved.team.name) && sameTeam(fixture.away,homeResolved.team.name))));
    if (match) {
      const reversed = sameTeam(match.home, awayResolved.team.name);
      if (reversed) {
        [analysis.teams.home, analysis.teams.away] = [analysis.teams.away, analysis.teams.home];
        [analysis.form.home, analysis.form.away] = [analysis.form.away, analysis.form.home];
        analysis.advanced = null; analysis.h2h = null; analysis.history = null; analysis.weather = null;
      }
      analysis.fixture = {
        ...(analysis.fixture || {}),
        inputReversed: reversed,
        id: match.id,
        date: match.date,
        league: match.league || analysis.fixture?.league || null,
        leagueId: match.leagueId || null,
        venue: match.venue || (reversed ? awayResolved : homeResolved).team.venue?.name || (!reversed && analysis.fixture?.venue) || null,
        city: match.city || (reversed ? awayResolved : homeResolved).team.venue?.city || (!reversed && analysis.fixture?.city) || null,
