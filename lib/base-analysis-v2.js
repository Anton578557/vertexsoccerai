...
function normalize(value) {
  return clean(value)
    ...
    .normalize('NFD')
    .replace(/[\u0300-\u036f]/g, '')
    // Common club prefixes/suffixes should not make a senior team lose to a
    // youth/reserve result when the user enters a short club name (e.g. Milan).
...
function teamCandidateScore(team, query) {
  ...
  if (!queryWantsYouthOrReserve(query) && youthOrReserveName(name)) score -= 0.7;
...
async function findTeam(name) {
  ...
  try {
    ...
    teams.sort((a, b) => teamCandidateScore(b, name) - teamCandidateScore(a, name));
    const team = teams[0];
    return {
      id: team.idTeam || null,
