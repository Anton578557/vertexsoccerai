...
function chooseTeamCandidate(teams, query) {
  ...
  if (queryWantsYouthOrReserve(query)) return ranked[0];

  const senior = ranked.find((team) => !youthOrReserveName(team?.strTeam) && scoreName(team?.strTeam, query) >= 0.58);
  if (senior) return senior;

  if (youthOrReserveName(ranked[0]?.strTeam)) return null;
  return ranked[0];
}
...
async function weatherForLocation(location) {
  ...
  try {
    const geoResult = await cachedProviderCall({
      cacheKey: `openweather:geo:v2:${safeKey(location)}`,
      provider: 'OpenWeather geocoding',
      ttlSeconds: 30 * 24 * 3600,
      staleSeconds: 90 * 24 * 3600,
      loader: async () => {
        const geo = await fetchJson(
          `https://api.openweathermap.org/geo/1.0/direct?q=${encodeURIComponent(clean(location, 100))}&limit=1&appid=${encodeURIComponent(key)}`,
          {},
          6000
        );
