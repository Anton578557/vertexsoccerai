...
const {footballCountry}=require('./football-country');

const { sameTeam } = require('./match-integrity');
const { resolveTeamName, ambiguousTeamChoices } = require('./team-aliases');
const { cachedProviderCall, getProviderCache } = require('./provider-cache');
const { searchClubDirectory, chooseExactClub } = require('./club-directory');
...
const TEAM_IDS = { 'Operario Ferroviario': '136829', 'Operario de Campo Grande': '147149', Ceara: '134744', Novorizontino: '141182', 'Sao Bernardo': '145389', 'Cambridge United': '134586', 'AFC Wimbledon': '134241', 'Atletico Junior': '137615', 'Real Tomayapo': '140725', 'Nottingham Forest': '133720', 'Racing Club': '135170', 'Boca Juniors': '135156', 'Blaublitz Akita': '140509', 'Albirex Niigata': '139881', Nautico: '134289', 'Sport Recife': '136250', 'Independiente Medellin': '137613', Millonarios: '137617' };
...
function chooseTeamCandidate(teams, query, aliases = []) {
  ...
  const candidates = teams.filter(team => [team.strTeam, ...(team.strAlternate || '').split(';')]
    .some(name => [query, ...aliases].some(alias => sameTeam(name, alias))));
...
async function findTeam(name, aliases = []) {
  if (ambiguousTeamChoices(name).length) return null;
  try {
    const { payload } = await cachedProviderCall({
      cacheKey: `thesportsdb:team:v8:${safeKey(name)}`,
      provider: 'TheSportsDB',
      ttlSeconds: 7 * 24 * 3600,
      staleSeconds: 30 * 24 * 3600,
      loader: async () => {
        const previous = await getProviderCache(`thesportsdb:team:v4:${safeKey(name)}`);
        if (!TEAM_IDS[resolveTeamName(name)] && previous?.country && previous?.badge && sameTeam(previous.name, name)) return previous;
        ...
        const team = chooseTeamCandidate(teams, name, aliases);
        if (!team) return null;
        return {
          id: team.idTeam || null,
          source: 'TheSportsDB',
...
async function resolveClub(input) {
  const name = resolveTeamName(input);
  if (ambiguousTeamChoices(name).length) return null;
  const direct = await findTeam(name);
  if (direct) return direct;
  const choices = await searchClubDirectory(input);
  const identity = chooseExactClub(choices, input);
  if (!identity) return null;
  const lookupName = resolveTeamName(identity.name).replace(/\b(?:A\.\s*F\.\s*C\.|F\.\s*C\.|C\.\s*F\.|S\.\s*C\.|C\.\s*D\.|U\.\s*D\.)/gi, part => part.replace(/[.\s]/g, '')).replace(/\b(?:FC|AFC|CF|SC|CD|UD)\b/g, ' ').replace(/\s+/g, ' ').trim();
  const metadata = await findTeam(lookupName, identity.aliases);
  return { ...identity, ...metadata, source: metadata ? 'TheSportsDB + Wikidata' : 'Wikidata' };
}
...
async function buildBaseAnalysis(homeInput, awayInput, original = {}) {
  ...
  const [homeMeta, awayMeta] = await Promise.all([resolveClub(original.home || homeInput), resolveClub(original.away || awayInput)]);
  const resolvedHome = resolveTeamName(homeMeta?.name || homeName);
  const resolvedAway = resolveTeamName(awayMeta?.name || awayName);
  if(homeMeta)homeMeta.country=footballCountry(homeMeta.country);
  if(awayMeta)awayMeta.country=footballCountry(awayMeta.country);
  ...
  return {
    ...
    teams: {
      home: { name: resolvedHome, badge: homeMeta?.badge || null, country: homeMeta?.country || null, resolved: Boolean(homeMeta), espnId: homeMeta?.espnId || null, aliases: homeMeta?.aliases || [], metadataSource: homeMeta?.source || null },
      away: { name: resolvedAway, badge: awayMeta?.badge || null, country: awayMeta?.country || null, resolved: Boolean(awayMeta), espnId: awayMeta?.espnId || null, aliases: awayMeta?.aliases || [], metadataSource: awayMeta?.source || null }
    },
    fixture: {
      date: null,
    ...
    sourceStatus: {
      ...
      teamMetadata: homeMeta && awayMeta ? [...new Set([homeMeta.source, awayMeta.source])].join(' / ') : 'Partial',
