...
const FIELDS = { HC:'corner_kicks', AC:'corner_kicks', HY:'yellow_cards', AY:'yellow_cards', HR:'red_cards', AR:'red_cards',
  HS:'total_shots', AS:'total_shots', HST:'shots_on_target', AST:'shots_on_target', HF:'fouls', AF:'fouls', HO:'offsides', AO:'offsides' };
...
function statsRow(event, payload) {
  ...
  for (const side of ['home','away']) {
    const xg=payload?.stats?.[side]?.xg;
    // The team aggregate has its own provenance flag; estimated shot maps do
    // not make a separately measured team total an estimated value (BSD docs).
    row[side==='home'?'HXG':'AXG']=xg?.estimated === false && typeof xg.actual === 'number' && Number.isFinite(xg.actual) && xg.actual>=0 && xg.actual<=15 ? xg.actual : null;
  }
  return row;
}

function measuredPerformance(rows, home, away) {
  const sides=Object.fromEntries([['home',home],['away',away]].map(([side,name])=>{
    const relevant=rows.filter(r=>(sameTeam(r.HomeTeam,name)||sameTeam(r.AwayTeam,name)) && r.HXG!=null && r.AXG!=null);
    const average=against=>relevant.reduce((s,r)=>s+(sameTeam(r.HomeTeam,name)!==against?r.HXG:r.AXG),0)/relevant.length;
    return [side,{sample:relevant.length,xgFor:relevant.length?Number(average(false).toFixed(2)):null,
      xgAgainst:relevant.length?Number(average(true).toFixed(2)):null}];
  }));
  return {source:'BSD measured team xG',...sides,available:Math.min(sides.home.sample,sides.away.sample)>=3,
    appliedToModel:false,reason:'collecting_validation_sample'};
}
...
async function loadModel(analysis, cutoff) {
  ...
  model.measuredXg=measuredPerformance(rows,analysis.teams.home.name,analysis.teams.away.name);
...
async function buildBsdEventModel(analysis) {
  ...
  try {
    const { payload } = await cachedProviderCall({cacheKey:`bsd-event-model:v2:${identity('home')}:${identity('away')}:${String(cutoff).slice(0,10)}`,
...
function mergeEventModels(primary, fallback) {
  ...
  merged.measuredXg = fallback.measuredXg;
  return merged;
}

module.exports = { buildBsdEventModel, statsRow, eligibleEvents, mergeEventModels, measuredPerformance };
