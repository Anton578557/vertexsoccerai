  ...
  const result = await cachedProviderCall({ cacheKey: `bsd:v1:${path}?${query}`, provider: 'BSD', ttlSeconds, staleSeconds: 0,
    loader: async () => {
      ...
      if (/^events\/\d+\/stats$/.test(path)) {
        ...
        return { event_id: data.event_id, stats: { home: data.stats.home, away: data.stats.away } };
      }
...
module.exports = { resolveTeam, teamHistory, requestMatchStats, enrichBsdHistory, configuration, selectTeam, normalizeEvents, retrySeconds, completedBsdMatches };
