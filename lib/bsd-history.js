...
const { providerAliases, ambiguousTeamChoices } = require('./team-aliases');
const { contextFromHistory, applyHistory } = require('./verified-history');
const { leagueCode } = require('./football-data-uk');
...
async function request(path, params, ttlSeconds = 3600) {
  ...
  const version = /\/stats$/.test(path) ? 'v2' : 'v1';
  const result = await cachedProviderCall({ cacheKey: `bsd:${version}:${path}?${query}`, provider: 'BSD', ttlSeconds, staleSeconds: 0,
    loader: async () => {
      ...
      if (/^events\/\d+\/stats$/.test(path)) {
        if (Number(data?.event_id) !== Number(path.split('/')[1]) || !data.stats || typeof data.stats !== 'object') throw new Error('invalid_dataset');
        // Persist only statistics, not shotmaps, player records or other large views.
        return { event_id: data.event_id, xg_estimated: data.xg_estimated ?? null,
          stats: { home: data.stats.home, away: data.stats.away } };
      }
      if (/^events\/\d+\/lineups$/.test(path)) {
        if (Number(data?.event_id) !== Number(path.split('/')[1])) throw new Error('invalid_dataset');
        return {event_id:data.event_id,lineup_status:data.lineup_status,beta:data.beta,lineups:data.lineups,
          unavailable_players:data.unavailable_players,updated_at:data.updated_at};
      }
      if (/^events\/\d+$/.test(path)) {
        if (Number(data?.id ?? data?.event_id) !== Number(path.split('/')[1])) throw new Error('invalid_dataset');
        return Object.fromEntries(['id','event_id','status','event_date','home_team_id','away_team_id','home_team','away_team',
          'league_id','referee_id','referee','referee_name','venue','venue_name','is_neutral_ground'].filter(key=>key in data).map(key=>[key,data[key]]));
      }
...
function selectTeam(rows, team, countryCode, aliases = []) {
  if (ambiguousTeamChoices(team.name).length) return null;
  ...
  const matches = (rows || []).filter(row => Number.isInteger(row.id) && row.id > 0 && row.is_women !== true &&
    ...
    (!countryCode || !row.country_code || row.country_code === countryCode) && names.some(name => sameTeam(name, row.name) || sameTeam(name,row.short_name)));
  const unique = new Map(matches.map(row => [row.id, row]));
  const chosen=unique.size === 1 ? [...unique.values()][0] : null;
  return chosen ? {...chosen,canonicalName:sameTeam(team.name,chosen.name)?team.name:chosen.name} : null;
}
...
async function resolveTeam(team) {
  ...
  for (const name of [...new Set([team.name, ...aliases, ...(team.aliases || [])])].filter(Boolean).slice(0, 3)) {
    ...
    const candidates=(data.results || []).filter(row=>selectTeam([row],team,countryCode,aliases));
    if(candidates.length>1 && candidates.length<=3) {
      // Duplicate directory IDs are common. Require recent membership in a
      // recognised senior league; a shorter name or a crest is not proof.
      const countries=[...new Set(candidates.map(row=>row.country).filter(Boolean))];
      if(countries.length===1) {
        const leagues=await request('leagues',{country:countries[0],is_women:'false',limit:'200',offset:'0'},86400);
        const ids=new Set((leagues.results || []).filter(l=>l.is_women!==true && leagueCode(l.name,countries[0])).map(l=>l.id));
        if(!leagues.next && ids.size) {
          const active=await Promise.all(candidates.map(async candidate=>{
            const history=await request('events',{team_id:String(candidate.id),status:'finished',
              date_from:new Date(Date.now()-60*864e5).toISOString().slice(0,10),date_to:new Date().toISOString().slice(0,10),limit:'200',offset:'0'});
            const valid=normalizeEvents(history.results,candidate,[candidate]).filter(e=>ids.has(e.league));
            return !history.next && valid.length>=3?candidate:null;
          }));
          const verified=active.filter(Boolean);
          if(verified.length===1)return selectTeam(verified,team,countryCode,aliases);
        }
      }
    }
  }
...
function normalizeEvents(events, team, knownTeams = [], now = new Date()) {
  return (events || []).flatMap(event => {
    ...
    return [{ id: `bsd:${event.id}`, date: event.event_date,
      ...
      homeScore: event.home_score, awayScore: event.away_score, regulationVerified: true, eventStatsRegulation: !event.extra_time_score && !event.penalty_shootout && !/extra|penalty|\bET\b|\bPEN\b/i.test(event.period || ''), source: 'BSD', league: event.league_id || null }];
  });
}
...
async function enrichBsdHistory(analysis, { verifyConnection = false } = {}) {
  ...
  try {
    ...
    teams.forEach((team, index) => {
      ...
      target.aliases=[...new Set([...(target.aliases || []),team.short_name,target.name].filter(Boolean))];
      target.name=team.canonicalName;
...
async function requestMatchStats(id) {
  if (!Number.isSafeInteger(id) || id <= 0) throw new Error('invalid_event_id');
  return request(`events/${id}/stats`, {}, 86400);
}

async function requestFootballResource(path, params = {}, ttl = 300) {
  if (!configuration().enabled || !configuration().configured) throw new Error('not_configured');
  if (!/^(?:events(?:\/\d+(?:\/lineups)?)?|leagues|referees\/\d+\/matches)$/.test(path)) throw new Error('unsupported_resource');
  return request(path, params, ttl);
}

module.exports = { resolveTeam, teamHistory, requestMatchStats, requestFootballResource, enrichBsdHistory, configuration, selectTeam, normalizeEvents, retrySeconds, completedBsdMatches };
