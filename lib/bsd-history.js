...
// Prepared against BSD v2 documentation. Explicitly opt in only AFTER the
// owner's free account and actual payload/coverage have been verified.
...
const COUNTRIES = { Brazil: 'BR', Colombia: 'CO', England: 'GB', Germany: 'DE', Spain: 'ES', Italy: 'IT', France: 'FR',
  Portugal: 'PT', Netherlands: 'NL', Argentina: 'AR', Belgium: 'BE', Scotland: 'GB', Turkey: 'TR', Mexico: 'MX', USA: 'US', Japan: 'JP' };
...
function selectTeam(rows, team, countryCode, aliases = []) {
  ...
  const matches = (rows || []).filter(row => Number.isInteger(row.id) && row.id > 0 && row.is_women !== true &&
    (!row.country_code || row.country_code === countryCode) && names.some(name => sameTeam(name, row.name)));
...
async function resolveTeam(team) {
  ...
  const aliases = team.country === 'Brazil' && sameTeam(team.name, 'Sport Recife') ? ['Sport'] : [];
  for (const name of [...new Set([team.name, ...aliases])]) {
    const data = await request('teams', { name, country_code: countryCode, is_women: 'false', limit: '200', offset: '0' }, 86400);
...
function normalizeEvents(events, team, knownTeams = [], now = new Date()) {
  return (events || []).flatMap(event => {
    if (event.status !== 'finished' || !Number.isInteger(event.id) || !event.home_team || !event.away_team ||
...
module.exports = { enrichBsdHistory, configuration, selectTeam, normalizeEvents, retrySeconds };
