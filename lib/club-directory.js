...
function entityClub(entity) {
  ...
  const football = values('P641').some(v => v?.id === 'Q2736') || values('P31').some(v => v?.id === 'Q476028') || /(?:association football|soccer|football club|футбольн|fútbol)/i.test(descriptions);
  const person = values('P31').some(v => v?.id === 'Q5');
  if (!football || person || /american football|rugby|futsal/i.test(descriptions)) return null;
...
async function searchClubDirectory(query) {
  ...
  try {
    const result = await cachedProviderCall({ cacheKey: `club-directory:v1:${cacheKey(input)}`, provider: 'Wikidata', ttlSeconds: 86400, staleSeconds: 7 * 86400,
      loader: async () => {
        ...
        return Object.values(data.entities || {}).map(entityClub).filter(Boolean);
      } });
