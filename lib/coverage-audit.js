...
async function coverageAudit(code) {
  ...
  // Only the Turkish case changed after the final matrix: provider display name Amed.
  const revision=code==='T1'?2:1;
  const {payload}=await cachedProviderCall({cacheKey:`coverage-audit:v5:${code}${revision>1?`:r${revision}`:''}`,provider:'Vertex Coverage Audit',ttlSeconds:86400,staleSeconds:0,
    loader:async()=>{
      ...
      return {league:LEAGUES[code],code,revision,timestamp:new Date().toISOString(),
