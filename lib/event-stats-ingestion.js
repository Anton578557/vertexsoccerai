...
function sameTeam(a, b) {
  ...
  return common / Math.max(xs.size, ys.size, 1) >= 0.58;
}
...
async function fetchSeason(code, season) {
  const response = await fetch(`${BASE}/${season}/${code}.csv`, {
    headers: { Accept: 'text/csv,*/*', 'User-Agent': 'VertexSoccerAI/4.0' }
  });
...
async function ingestFootballDataUkStats({ code, seasons, homeName, awayName, competition }) {
  ...
  for (const season of seasons.slice(-2)) {
    try {
      ...
      recentRelevant(rows, homeName, awayName, 12).forEach((row) => {
