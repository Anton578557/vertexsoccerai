...
function normalize(value) { return clean(resolveTeamName(value)).toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/\b(fc|cf|afc|sc|club|de|the|ud|cd)\b/g, ' ').replace(/[^a-z0-9]+/g, ' ').replace(/\s+/g, ' ').trim(); }
...
function sameTeam(a, b) {
  ...
  return common / Math.max(xs.size, ys.size, 1) >= 0.8;
}
...
function competitionCode(league) {
  ...
  const rules = [
    [/champions league|uefa champions/, 'CL'],
    [/europa league|uefa europa|uefa cup/, 'EL'],
    [/conference league|uefa conference/, 'UCL'],
    ...
    [/primeira liga|liga portugal/, 'PPL'], [/championship/, 'ELC'],
...
module.exports = { sameTeam, enrichFootballData, resolveUpcomingFixture, competitionCode, advancedStats, buildLeagueContext, buildH2h, buildPenaltyContext };