...
async function fetchJson(url) {
  ...
  try {
    const response = await fetch(url, {
      ...
      headers: { Accept: 'application/json', 'X-Auth-Token': process.env.FOOTBALL_DATA_KEY, 'User-Agent': 'VertexSoccerAI/4.1' }
    });
    if (!response.ok) {
...
async function enrichFootballData(homeName, awayName, leagueName) {
  ...
  try {
    ...
    const { payload: data } = await cachedProviderCall({
      cacheKey: `football-data:competition:${code}:${fromIso}:${toIso}`,
