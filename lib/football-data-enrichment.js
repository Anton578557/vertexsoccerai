...
async function fetchJson(url) {
  ...
  try {
    const response = await fetch(url, {
      ...
      headers: { Accept: 'application/json', 'X-Auth-Token': process.env.FOOTBALL_DATA_KEY, 'User-Agent': 'VertexSoccerAI/5.0' }
    });
    if (!response.ok) {
...
function matchView(match, team) {
  ...
  return {
    ...
    opponent: opponentName(match, team)
  };
}
...
function advancedStats(events, team, table, fixtureDate, expectedVenue) {
  const recent = completedFor(events, team, 12).map((match, index) => {
  ...
  return {
    ...
    cleanSheetRate,
    restDays,
    matches7: countWindow(7),
    matches14: countWindow(14),
    matches30: countWindow(30),
    venue: {
      type: expectedVenue,
      matches: venueRows.length,
      avgFor: venueAvgFor,
      avgAgainst: venueAvgAgainst,
      ppg: venuePpg
    }
  };
}
...
function buildLeagueContext(completed) {
  ...
  return {
    sample: n,
    homeGoals: homeGoals / n,
    awayGoals: awayGoals / n,
    teamGoalAvg: (homeGoals + awayGoals) / (2 * n),
...
function buildH2h(completed, homeName, awayName) {
  ...
  return {
    sample: rows.length,
    homePpg: weightTotal ? homePoints / weightTotal : null,
    awayPpg: weightTotal ? awayPoints / weightTotal : null,
    homeGoalDiff: weightTotal ? (homeGoals - awayGoals) / weightTotal : null
  };
}
...
async function enrichFootballData(homeName, awayName, leagueName) {
  ...
  try {
    ...
    const { payload: data } = await cachedProviderCall({
      cacheKey: `football-data:v2:competition:${code}:${fromIso}:${toIso}`,
    ...
    const completed = all.filter((m) => Number.isFinite(m.homeScore) && Number.isFinite(m.awayScore));
    ...
    const homeEvents = completedFor(relevant, homeName, 12); const awayEvents = completedFor(relevant, awayName, 12);
    ...
    const advanced = {
      home: advancedStats(relevant, homeName, table, fixtureDate, 'home'),
      away: advancedStats(relevant, awayName, table, fixtureDate, 'away')
    };
...
module.exports = { enrichFootballData, competitionCode, advancedStats, buildLeagueContext, buildH2h };
