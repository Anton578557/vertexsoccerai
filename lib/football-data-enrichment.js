...
function completedFor(events, team, limit = 12, reference = Date.now()) {
  const cutoff = Math.min(Date.now(), new Date(reference).getTime());
  return events.filter((m) => new Date(m.date).getTime() < cutoff && new Date(m.date).getTime() > cutoff - 365 * 864e5 && (sameTeam(m.home, team) || sameTeam(m.away, team)) && Number.isFinite(m.homeScore) && Number.isFinite(m.awayScore))
    .sort((a, b) => new Date(b.date || 0) - new Date(a.date || 0)).slice(0, limit);
}

function basicStats(events, team, limit = 8, reference = Date.now()) {
  const rows = completedFor(events, team, limit, reference);
...
function advancedStats(events, team, table, fixtureDate, expectedVenue) {
  ...
  const recent = completedFor(events.filter(m => new Date(m.date).getTime() < cutoff), team, 12, cutoff).map((match, index) => {
    const view = matchView(match, team);
    const opponent = table.get(normalize(view?.opponent)) || null;
    const opponentVerified = Number.isFinite(opponent?.ppg) && opponent.played >= 3;
    const opponentPpg = opponentVerified ? opponent.ppg : 1.35;
    const recencyWeight = Math.pow(0.87, index);
    const oppositionWeight = clamp(0.82 + (opponentPpg / 3) * 0.36, 0.82, 1.18);
    return { ...view, opponentPpg, opponentVerified, weight: recencyWeight * oppositionWeight, recencyWeight };
  }).filter(Boolean);
  ...
  return {
    ...
    opponentSample: recent.filter(row => row.opponentVerified).length,
    restReferenceDate: new Date(targetTime).toISOString(),
...
function teamIdentityFromMatches(matches, name) {
  const hits = new Map();
  for (const match of matches || []) for (const team of [match.homeTeam, match.awayTeam]) {
    if (Number.isInteger(team?.id) && sameTeam(team.name,name)) hits.set(team.id,team);
  }
  return hits.size === 1 ? [...hits.values()][0] : null;
}

async function recoverFootballDataMetadata(analysis, identities = {}) {
  await Promise.all(['home','away'].map(async side => {
    const team = analysis.teams[side], identity = identities[side];
    if (!Number.isInteger(identity?.id) || !sameTeam(identity.name,team.name)) return;
    if (team.resolved && team.badge && team.country) return;
    let country = team.country;
    // Match area describes the competition, not necessarily the club's country.
    // Read the exact team resource instead of guessing from the other side.
    if (!country) try {
      const {payload} = await cachedProviderCall({cacheKey:`football-data:team-info:v1:${identity.id}`,provider:'Football-Data',ttlSeconds:86400,staleSeconds:7*86400,
        loader:()=>fetchJson(`${BASE}/teams/${identity.id}`)});
      if (payload?.id === identity.id && sameTeam(payload.name,team.name)) country = clean(payload.area?.name) || null;
    } catch (_) { /* Keep country missing and the crest independently usable. */ }
    analysis.teams[side] = {...team, country:country || null, resolved:true, footballDataId:identity.id,
      badge:team.badge || (/^https:\/\//.test(identity.crest || '') ? identity.crest : null),
      aliases:[...new Set([...(team.aliases || []),identity.name])], metadataSource:'Football-Data'};
  }));
  analysis.sourceStatus ||= {};
  if (Object.values(analysis.teams).some(team=>team.metadataSource === 'Football-Data')) {
    analysis.sourceStatus.teamMetadata = [...new Set(Object.values(analysis.teams).map(team=>team.metadataSource || (team.resolved ? 'TheSportsDB' : 'Partial')))].join(' + ');
  }
  return analysis;
}
...
async function enrichFootballData(homeName, awayName, leagueName, country = '') {
  ...
  try {
    ...
    return {
      ...
      homeEvents,
      awayEvents,
      teamIdentities: {home:teamIdentityFromMatches(data?.matches,homeName),away:teamIdentityFromMatches(data?.matches,awayName)},
      homeForm: basicStats(homeEvents, homeName, 8),
      awayForm: basicStats(awayEvents, awayName, 8),
      history: { source: 'Football-Data', home: homeEvents, away: awayEvents },
      fixture,
      advanced,
...
module.exports = { basicStats, leagueTable, enrichFootballData, resolveUpcomingFixture, competitionCode, advancedStats, buildLeagueContext, buildH2h, buildPenaltyContext, teamIdentityFromMatches, recoverFootballDataMetadata };
