...
const PARAMETERS=require('./model-parameters.json');
const {countOver}=require('./count-distribution');
const BASE = 'https://www.football-data.co.uk/mmz4281';
const COUNTRY_CODES={england:['E0','E1','E2','E3'],germany:['D1','D2'],spain:['SP1','SP2'],italy:['I1','I2'],france:['F1','F2'],
  portugal:['P1'],netherlands:['N1'],belgium:['B1'],scotland:['SC0'],greece:['G1'],turkey:['T1'],brazil:['BRA'],argentina:['ARG'],japan:['JPN']};
const LEAGUE_NAMES={E0:'English Premier League',E1:'English Championship',E2:'English League One',E3:'English League Two',
  D1:'German Bundesliga',D2:'2. Bundesliga',SP1:'Spanish La Liga',SP2:'Segunda Division',I1:'Italian Serie A',I2:'Italian Serie B',
  F1:'French Ligue 1',F2:'French Ligue 2',P1:'Primeira Liga',N1:'Eredivisie',B1:'Belgian Pro League',SC0:'Scottish Premiership',
  G1:'Greek Super League',T1:'Turkish Super Lig',BRA:'Brazilian Serie A',ARG:'Liga Profesional Argentina',JPN:'Japan J1 League'};
...
function leagueCode(leagueName, country = '') {
  ...
  const rules = [
    ['E1', 'england', /championship/], ['E2', 'england', /^(?:english )?league (?:one|1)$|^league 1 england$/],
    ['E3', 'england', /^(?:english )?league (?:two|2)$|^league 2 england$/], ['E0', 'england', /^(english )?premier league$/],
    ...
    ['B1', 'belgium', /jupiler|belgian first|pro league/], ['SC0', 'scotland', /scottish premiership|scottish premier league|premiership scotland|^premiership$/],
    ['G1', 'greece', /greek super|super\s?league greece|^super league(?: 1)?$/], ['T1', 'turkey', /turkish super|super lig/]
  ];
...
function historyContext(rows, homeName, awayName, cutoff = new Date()) {
  ...
  const homeForm = basicStats(homeEvents, homeName, 8, cutoff), awayForm = basicStats(awayEvents, awayName, 8, cutoff);
...
async function buildOpenHistoricalContext(homeName, awayName, leagueName, country, fixtureDate) {
  ...
  const load=async candidate=>{
    const {rows}=await loadHistoricalRows(candidate,fixtureDate);
    return {...historyContext(rows,homeName,awayName,fixtureDate || new Date()),code:candidate,leagueName:LEAGUE_NAMES[candidate]};
  };
  const first=code?await load(code):{ok:false,reason:'league_not_covered'};
  const latest=result=>Math.min(...['home','away'].map(side=>Math.max(0,...(result.history?.[side] || []).map(r=>Date.parse(r.date)||0))));
  const recent=result=>result.ok && latest(result)>Math.min(Date.now(),Date.parse(fixtureDate || new Date()))-30*864e5;
  if(recent(first))return first;
  // Metadata can lag promotion/relegation. Search only verified divisions in
  // the same country; never send a generic 'Premier League' across countries.
  const alternatives=(COUNTRY_CODES[String(country || '').toLowerCase()] || []).filter(c=>c!==code);
  const candidates=await Promise.all(alternatives.map(load));
  return [first,...candidates].filter(r=>r.ok).sort((a,b)=>latest(b)-latest(a))[0] || first;
}
...
function teamProfile(rows, teamName) {
  ...
  const collect = (homeFor, awayFor, homeAgainst, awayAgainst) => {
    const values = sample.map((row) => ({...metricForTeam(row, teamName, homeFor, awayFor, homeAgainst, awayAgainst), date: rowDate(row)}))
      .filter(v => v && [v.for, v.against].every(value => Number.isInteger(value) && value >= 0));
    return {
      for: average(values.map((v) => v.for)),
      against: average(values.map((v) => v.against)),
      n: values.length,
      through: sampleDate(values, 'latest'), from: sampleDate(values, 'earliest')
    };
  };
...
function sampleDate(values, mode) {
  const dates = values.map(v => v.date).filter(Number.isFinite);
  return dates.length ? new Date((mode === 'latest' ? Math.max : Math.min)(...dates)).toISOString() : null;
}

function freshMetric(metric, cutoff = new Date()) {
  const end = Math.min(Date.now(), Date.parse(cutoff));
  const last = Date.parse(metric?.through);
  return metric?.n >= 3 && Number.isFinite(last) && last < end && end - last <= 90 * 864e5;
}

function expectedPair(homeProfile, awayProfile, metric, cutoff) {
  if (!freshMetric(homeProfile?.[metric], cutoff) || !freshMetric(awayProfile?.[metric], cutoff)) return {home:null,away:null,total:null};
...
function redCardHistory(rows, teamName) {
  const sample = teamRows(rows, teamName, 12).map(row => ({...metricForTeam(row, teamName, 'HR', 'AR', 'AR', 'HR'), date: rowDate(row)}))
    .filter(v => v && [v.for, v.against].every(value => Number.isInteger(value) && value >= 0));
  if (sample.length < 3) return null;
  const matchesWithRed = sample.filter(v => v.for > 0).length;
  return { sample: sample.length, matchesWithRed, observedFrequency: round(matchesWithRed / sample.length * 100, 1),
    average: round(average(sample.map(v => v.for))), from: sampleDate(sample, 'earliest'), through: sampleDate(sample, 'latest') };
}
...
async function buildGranularHistoricalModel(homeName, awayName, leagueName, fixtureDate, country = '') {
  ...
  return buildGranularFromRows(rows, homeName, awayName, { code, seasons, fixtureDate });
}

// Both providers use the same count model, paired samples and no-future-data guard.
function buildGranularFromRows(inputRows, homeName, awayName, { code = null, seasons = [], fixtureDate = new Date(), source = 'Football-Data.co.uk',countParameters = PARAMETERS } = {}) {
  const rows = historicalRows(inputRows, fixtureDate);
  if (!rows.length) return { ok: false, reason: 'No verified historical event statistics returned.', source, code };

  const home = teamProfile(rows, homeName);
  const away = teamProfile(rows, awayName);
  if (!home.matches || !away.matches) {
    return { ok: false, reason: 'Teams were not matched in the historical CSV.', source, code };
  }

  const corners = expectedPair(home, away, 'corners', fixtureDate);
  const cards = expectedPair(home, away, 'yellowCards', fixtureDate);
  const redCards = { home: redCardHistory(rows, homeName), away: redCardHistory(rows, awayName) };
  const shots = expectedPair(home, away, 'shots', fixtureDate);
  const shotsOnTarget = expectedPair(home, away, 'shotsOnTarget', fixtureDate);
  const offsides = expectedPair(home, away, 'offsides', fixtureDate);
  const fouls = expectedPair(home, away, 'fouls', fixtureDate);

  const coverage = {
    corners: Number.isFinite(corners.total),
    cards: Number.isFinite(cards.total),
    redCards: Boolean(redCards.home || redCards.away),
  ...
  return {
    ok: Object.values(coverage).some(Boolean) || home.yellowCards.n >= 3 || away.yellowCards.n >= 3,
    forecastsAvailable: [corners, cards, shots, shotsOnTarget, offsides, fouls].some(metric => Number.isFinite(metric.total)),
    yellowHistory: { home: home.yellowCards.n >= 3 ? home.yellowCards : null, away: away.yellowCards.n >= 3 ? away.yellowCards : null },
    source,
    code,
    seasons,
    sample: { home: home.matches, away: away.matches },
    eventSamples: Object.fromEntries([['corners','corners'],['cards','yellowCards'],['redCards','redCards']].map(([key,metric]) => [key,{home:home[metric].n,away:away[metric].n}])),
    quality,
    ...
    corners: coverage.corners ? {
      ...
      over75: pct(countOver(corners.total, 7.5,countParameters.cornersDispersion)), over85: pct(countOver(corners.total, 8.5,countParameters.cornersDispersion)),
      over95: pct(countOver(corners.total, 9.5,countParameters.cornersDispersion)), over105: pct(countOver(corners.total, 10.5,countParameters.cornersDispersion))
    } : null,
    cards: coverage.cards ? {
      sample: { home: home.yellowCards.n, away: away.yellowCards.n },
      expectedHome: round(cards.home), expectedAway: round(cards.away), expectedTotal: round(cards.total),
      over35: pct(countOver(cards.total, 3.5,countParameters.yellowDispersion)), over45: pct(countOver(cards.total, 4.5,countParameters.yellowDispersion)),
      over55: pct(countOver(cards.total, 5.5,countParameters.yellowDispersion))
    } : null,
    redCards: coverage.redCards ? { ...redCards, methodology: 'observed_history_only' } : null,
...
async function completedCsvMatches(league, country, date) {
  ...
  return rows.filter(row => new Date(rowDate(row)).toISOString().slice(0,10) === date).map(row => ({
    ...
    eventCounts:{corners:[num(row,'HC'),num(row,'AC')].every(Number.isInteger)?num(row,'HC')+num(row,'AC'):null,
      yellowCards:[num(row,'HY'),num(row,'AY')].every(Number.isInteger)?num(row,'HY')+num(row,'AY'):null}
  }));
}

module.exports = { buildOpenHistoricalContext, historyContext, buildGranularHistoricalModel, buildGranularFromRows, leagueCode, seasonCode, parseCsv, num, rowDate, historicalRows, fetchSeason, teamProfile, redCardHistory, freshMetric, expectedPair, completedCsvMatches };
