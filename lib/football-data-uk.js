...
function teamProfile(rows, teamName) {
  ...
  const collect = (homeFor, awayFor, homeAgainst, awayAgainst) => {
    const values = sample.map((row) => metricForTeam(row, teamName, homeFor, awayFor, homeAgainst, awayAgainst))
...
function redCardHistory(rows, teamName) {
  const sample = teamRows(rows, teamName, 12).map(row => metricForTeam(row, teamName, 'HR', 'AR', 'AR', 'HR'))
    .filter(v => v && [v.for, v.against].every(value => Number.isInteger(value) && value >= 0));
  ...
  return { sample: sample.length, matchesWithRed, observedFrequency: round(matchesWithRed / sample.length * 100, 1),
    average: round(average(sample.map(v => v.for))) };
}
...
module.exports = { buildOpenHistoricalContext, historyContext, buildGranularHistoricalModel, leagueCode, seasonCode, parseCsv, num, rowDate, historicalRows, fetchSeason, teamProfile, redCardHistory, completedCsvMatches };
