...
async function rapidRequest(path) {
  ...
  if (paused) { const error = new Error(paused.reason); error.status = paused.status; throw error; }
  let lastError = null;
  for (const host of rapidHosts()) {
    try {
      ...
      lastError = error;
      await recordProviderFailure('rapidApi',error.status,error.details || error.message);
