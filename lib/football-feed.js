...
function rapidHosts() {
  ...
  const out = [];
  if (validRapidHost(configured)) out.push(configured);
  if (!out.includes(RAPID_DEFAULT_HOST)) out.push(RAPID_DEFAULT_HOST);
  return out;
}
...
async function rapidRequest(path) {
  ...
  for (const host of rapidHosts()) {
    try {
      ...
      lastError = error;
      // 401/403 frequently means a stale host/subscription pairing. Try the
      // current known host automatically before declaring the provider down.
