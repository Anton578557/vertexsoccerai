...
function rapidHost() {
  ...
  if (!host) return null;
  if (!/^[a-z0-9.-]+\.p\.rapidapi\.com$/.test(host)) return null;
  return host;
}
...
async function rapidLiveMatches() {
  ...
  const data = await fetchJson(`https://${host}/${RAPID_LIVE_ENDPOINT}`, {
    headers: {
  ...
  return normalizeRapidLive(data);
}
...
async function sportmonksUpcoming(days = 7) {
  ...
  const result = await sportmonksGet(`/fixtures/between/${isoDate(from)}/${isoDate(to)}`, { include: 'participants;league;state' });
  if (!result.ok) return null;
...
async function footballDataUpcoming(days = 7) {
  ...
  const data = await fetchJson(
    `${FOOTBALL_DATA_BASE}/matches?dateFrom=${isoDate(from)}&dateTo=${isoDate(to)}`,
