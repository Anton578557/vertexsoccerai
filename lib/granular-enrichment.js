...
async function enhanceGranularAnalysis(analysis) {
  ...
  try {
    const { payload } = await cachedProviderCall({
      cacheKey: `granular:v3:${leagueKey}:${pairKey}:${dateKey}`,
  ...
  analysis.sourceStatus = {
    ...(analysis.sourceStatus || {}),
    footballDataCoUk: granular?.ok ? `Connected · ${granular.code || 'mapped league'}` : (granular?.reason || 'Event statistics unavailable; result history is checked separately')
  };
