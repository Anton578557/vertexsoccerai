...
async function enhanceGranularAnalysis(analysis) {
  ...
  try {
    const { payload } = await cachedProviderCall({
      cacheKey: `granular:v7:${safeKey(analysis.teams.home.country || '')}:${leagueKey}:${pairKey}:${dateKey}`,
  ...
  if(bsdModel?.measuredXg) analysis.performance={...(analysis.performance || {}),measuredXg:bsdModel.measuredXg};
