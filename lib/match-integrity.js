'use strict';

const { resolveTeamName, ambiguousTeamChoices } = require('./team-aliases');

// Resolve known aliases first; never identify two clubs by one shared token.
function buildTeamIdentity(value) {
  const stripped = resolveTeamName(String(value || '').replace(/\b(?:A\.\s*F\.\s*C\.|F\.\s*C\.|C\.\s*F\.|S\.\s*C\.|C\.\s*D\.|U\.\s*D\.)/gi, part => part.replace(/[.\s]/g, ''))).replace(/\b(?:FC|CF|AFC|SC|CD|UD)\b/gi, ' ').replace(/\s+/g, ' ').trim();
  return resolveTeamName(stripped).toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/\b(?:fc|cf|afc|sc|cd|ud)\b/g, ' ').replace(/[^a-z0-9]+/g, ' ').replace(/\s+/g, ' ').trim();
}

const identityCache=new Map();
function identity(value) {
  const key=String(value || '');
  if(identityCache.has(key)) return identityCache.get(key);
  const entry={name:buildTeamIdentity(key),ambiguous:Boolean(ambiguousTeamChoices(key).length)};
  if(identityCache.size>=5000)identityCache.delete(identityCache.keys().next().value);
  if(key.length<=200)identityCache.set(key,entry);
  return entry;
}
function teamIdentity(value){return identity(value).name;}

function sameTeam(a, b) {
  // Stripping CD must not turn Portuguese CD Operário into an exact match for
  // the ambiguous Brazilian/Portuguese shorthand Operário.
  const left=identity(a),right=identity(b);
  return Boolean(left.ambiguous===right.ambiguous && left.name && left.name===right.name);
}

// Football-data fullTime is also the running score and may include extra time.
function regulationScore(match) {
  if (String(match?.status).toUpperCase() !== 'FINISHED') return null;
  const score = match.score || {};
  const valid = (pair) => pair && ['home', 'away'].every((key) => Number.isInteger(pair[key]) && pair[key] >= 0);
  if (valid(score.regularTime)) return score.regularTime;
  if ((!score.duration || score.duration === 'REGULAR') && valid(score.fullTime)) return score.fullTime;
  return null;
}

module.exports = { sameTeam, teamIdentity, regulationScore };
