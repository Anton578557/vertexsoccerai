...
function selections(analysis) {
  ...
  for(const [market,key,field] of [['CORNERS_OU_9_5','corners','over95'],['YELLOWS_OU_4_5','cards','over45']]) {
    const over=clampPct(analysis.granularModel?.[key]?.[field]);
    if(over!=null) out.push({market,value:over>=50?'OVER':'UNDER',probability:over>=50?over:100-over});
  }
...
async function recordModelEvaluations(analysis) {
  ...
  const version = analysis.engine?.modelVersion || analysis.vertexModel?.version || analysis.model.engineVersion || null;
  const forecast = {version, generatedAt:analysis.generatedAt, fixture:analysis.fixture,
    teams:Object.fromEntries(['home','away'].map(side => [side, {name:analysis.teams[side].name,country:analysis.teams[side].country || null,aliases:analysis.teams[side].aliases || []}])),
    model:analysis.model, granular:analysis.granularModel || null,
    lineupStatus:analysis.matchContext?.lineups?.status || 'unavailable',
    dataQuality:quality, sources:analysis.contextSources || {},
    coverage:analysis.vertexModel?.coverage || {},
    samples:{home:analysis.form?.home?.played || 0,away:analysis.form?.away?.played || 0}};
  ...
  try {
    ...
    const recorded = Array.isArray(inserted) ? inserted.length : 0;
    // Successful ignore-duplicates means the original snapshot already exists.
    return { recorded, existing: recorded === 0, reason: recorded ? 'recorded' : 'first_snapshot_preserved' };
  } catch (error) {
    return { recorded: 0, reason: error?.message || 'write_failed' };
  }
}

module.exports = { recordModelEvaluations, fixtureKey, selections };
