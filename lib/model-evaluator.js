...
async function attachFinalEventCounts(match,forecast,date,deadline) {
  ...
  try {
    ...
    if(agree(csv) && Number.isInteger(csv.eventCounts?.corners) && Number.isInteger(csv.eventCounts?.yellowCards)) return {...match,eventCounts:csv.eventCounts,eventSource:csv.source};
    if(Date.now()>deadline) return match;
    const bsd=findMatch(await completedBsdMatches(home,away,date),home.name,away.name);
    if(!agree(bsd) || bsd.eventStatsRegulation!==true) return csv && agree(csv)?{...match,eventCounts:csv.eventCounts,eventSource:csv.source}:match;
    ...
    return {...match,eventCounts:{corners:sum(row.HC,row.AC),yellowCards:sum(row.HY,row.AY)},eventSource:'BSD'};
    ...
    const results = await Promise.all(entries.slice(i,i+2).map(async group => {
      ...
      await Promise.all(group.map(async row => {
        ...
        if (result) Object.assign(values, {actual_value:`${result.actual} · ${result.score}`,
          ...
          evaluation_status:'evaluated', evaluation_source:/CORNERS|YELLOWS/.test(row.market)?match.eventSource || match.source:match.source || 'Football-Data'});
