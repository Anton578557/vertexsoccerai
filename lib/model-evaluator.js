...
async function footballDataMatches(date) {
  ...
  return (payload?.matches || []).flatMap((match) => {
    ...
    return score ? [{ status: 'FINISHED', home: match.homeTeam?.name || '', away: match.awayTeam?.name || '', homeScore: score.home, awayScore: score.away }] : [];
  });
}
...
async function evaluatePendingModels(limit = 60) {
  ...
  const rows = await supabaseSelect('model_evaluations', {
    select: 'id,fixture_key,fixture_date,market,predicted_value,created_at',
    actual_value: 'is.null',
    fixture_date: `lt.${cutoff}`,
    order: 'fixture_date.asc',
    limit: String(Math.max(1, Math.min(Number(limit) || 60, 80)))
  });
  if (!rows.length) return { evaluated: 0, checkedFixtures: 0 };

  // Keep each serverless maintenance pass bounded to a few dates so it stays
  // inside Hobby function time limits. The next run continues the backlog.
  const dates = [...new Set(rows.map((row) => isoDate(row.fixture_date)).filter(Boolean))].slice(0, 4);
  const fetched = await Promise.all(dates.map(async (date) => {
    try { return [date, await footballDataMatches(date)]; }
    catch (_) { return [date, []]; }
  }));
  const matchesByDate = new Map(fetched);

  const updates = [];
  const seenFixtures = new Set();
  for (const row of rows) {
    if (!(Date.parse(row.created_at) < Date.parse(row.fixture_date))) continue;
    ...
    updates.push({
      ...
      values: {
        ...
        is_correct: predictionCorrect(row.market, row.predicted_value, result.actual),
  ...
  for (let offset = 0; offset < updates.length; offset += 16) {
    const batch = updates.slice(offset, offset + 16);
    const results = await Promise.all(batch.map((update) => patchEvaluation(update.id, update.values).catch(() => false)));
    evaluated += results.filter(Boolean).length;
  }

  return { evaluated, checkedFixtures: seenFixtures.size, pendingRowsChecked: rows.length };
}
...
function summarizeEvaluations(input) {
  ...
  return {
    ...
    evaluatedModelVersions: 'mixed_or_unversioned'
  };
}
...
async function performanceSummary() {
  ...
  const rows = await supabaseSelect('model_evaluations', {
    select: 'is_correct,data_quality,market,predicted_probability,fixture_key,fixture_date,created_at',
    is_correct: 'not.is.null', order: 'fixture_date.desc', limit: '10000'
  });
  return summarizeEvaluations(rows);
}

module.exports = { evaluatePendingModels, performanceSummary, findMatch, actualForMarket, summarizeEvaluations };
