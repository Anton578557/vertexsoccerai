...
const {boundedFetch:fetch,withBudget}=require('./analysis-budget');
...
const { completedBsdMatches,requestMatchStats } = require('./bsd-history');
const {statsRow}=require('./bsd-event-stats');
...
function actualForMarket(market, match) {
  if(market === 'CORNERS_OU_9_5' || market === 'YELLOWS_OU_4_5') {
    const total=match.eventCounts?.[market === 'CORNERS_OU_9_5'?'corners':'yellowCards'];
    if(match.status!=='FINISHED' || !Number.isInteger(total) || total<0) return null;
    return {actual:total>(market==='CORNERS_OU_9_5'?9.5:4.5)?'OVER':'UNDER',score:String(total)};
  }
...
function mergeFinalCounts(match, counts, source) {
  const result={...match,eventCounts:{...(match.eventCounts || {})},eventSources:{...(match.eventSources || {})}};
  for(const key of ['corners','yellowCards']) if(!Number.isInteger(result.eventCounts[key]) && Number.isInteger(counts?.[key]) && counts[key]>=0) {
    result.eventCounts[key]=counts[key];result.eventSources[key]=source;
  }
  return result;
}

async function attachFinalEventCounts(match,forecast,date,deadline) {
  if(!match || Date.now()>deadline) return match;
  const home=forecast?.teams?.home,away=forecast?.teams?.away;
  if(!home?.name || !away?.name) return match;
  const agree=row=>row && row.homeScore===match.homeScore && row.awayScore===match.awayScore && row.reversed===match.reversed;
  let result=match;
  try {
    const csv=findMatch(await completedCsvMatches(forecast.fixture?.league || '',home.country || '',date),home.name,away.name);
    if(agree(csv)) result=mergeFinalCounts(result,csv.eventCounts,csv.source);
    if(Number.isInteger(result.eventCounts?.corners) && Number.isInteger(result.eventCounts?.yellowCards)) return result;
    if(Date.now()>deadline) return result;
    const bsd=findMatch(await completedBsdMatches(home,away,date),home.name,away.name);
    if(!agree(bsd) || bsd.eventStatsRegulation!==true) return result;
    const row=statsRow(bsd,await requestMatchStats(Number(bsd.id.slice(4))));
    if(!row) return result;
    const sum=(a,b)=>Number.isInteger(a)&&Number.isInteger(b)?a+b:null;
    return mergeFinalCounts(result,{corners:sum(row.HC,row.AC),yellowCards:sum(row.HY,row.AY)},'BSD');
...
function evaluatePendingModels(limit = 60) {
  return withBudget(45000,()=>evaluateWithinBudget(limit));
}

async function evaluateWithinBudget(limit) {
  ...
  for (let i = 0; i < entries.length && Date.now() < deadline; i += 2) {
    const results = await Promise.all(entries.slice(i,i+2).map(async group => {
      ...
      let match = await resolveFinalMatch(first.forecast || fallback,date,await matchesByDate.get(date),{},deadline);
      if(group.some(row=>['CORNERS_OU_9_5','YELLOWS_OU_4_5'].includes(row.market))) match=await attachFinalEventCounts(match,first.forecast || fallback,date,deadline);
      ...
      await Promise.all(group.map(async row => {
        ...
        if (result) Object.assign(values, {actual_value:`${result.actual} · ${result.score}`,
          ...
          evaluation_status:'evaluated', evaluation_source:/CORNERS|YELLOWS/.test(row.market)?match.eventSources?.[row.market.startsWith('CORNERS')?'corners':'yellowCards'] || match.eventSource || match.source:match.source || 'Football-Data'});
...
async function performanceSummary() {
  ...
  summary.byLeague=Object.fromEntries([...new Set(rows.map(r=>r.forecast?.fixture?.league || 'unknown'))].map(league=>
    [league,summarizeEvaluations(rows.filter(r=>(r.forecast?.fixture?.league || 'unknown')===league))]));
  return summary;
}

module.exports = { mergeFinalCounts, evaluatePendingModels, performanceSummary, findMatch, actualForMarket, summarizeEvaluations, resolveFinalMatch, footballDataMatches };
