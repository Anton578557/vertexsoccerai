...
async function newsSource(provider, load) {
  ...
  try {
    ...
    return {provider,status,articles:[]};
  }
}
...
async function getNewsIntelligence(homeName, awayName) {
  ...
  try {
    const {payload}=await cachedProviderCall({cacheKey:`news:intelligence:v8:${pair}`,provider:'Football news',
      ttlSeconds:900,staleSeconds:0,loader:async()=>{
        const names=[...newsNames(homeName),...newsNames(awayName)];
        const q=names.map(name=>`"${name}"`).join(' OR ');
        const primary=process.env.NEWSAPI_KEY ? await newsSource('NewsAPI',async()=>{
          const params=new URLSearchParams({q,searchIn:'title,description',language:'en',sortBy:'publishedAt',
            pageSize:'40',from:isoDate(Date.now()-14*864e5)});
        ...
        const providers=[{provider:primary.provider,status:primary.status,received:primary.articles.length,accepted:candidates.length}];
        ...
        if(!candidates.length) {
          const fallback=await newsSource('GDELT',async()=>{
            const params=new URLSearchParams({query:`(${q}) (football OR soccer OR striker OR midfielder) sourcelang:english`,
              mode:'artlist',format:'json',timespan:'7d',maxrecords:'40',sort:'datedesc'});
            const data=await fetchJson(`https://api.gdeltproject.org/api/v2/doc/doc?${params}`,{},3000);
          ...
          providers.push({provider:fallback.provider,status:fallback.status,received:fallback.articles.length,accepted:candidates.length});
        }
