...
function classify(article, homeName, awayName) {
  ...
  if ((negativeStrong || negativeSoft || positiveStrong) && !teams.some(side => lead.includes(` ${normalize(side === 'home' ? homeName : awayName)} `))) return null;
  ...
  return { title, summary, source: article.source?.name || 'News', publishedAt: article.publishedAt || null, teams, signal, impact: 0, importance, relevanceVerified: true };
}
...
async function getNewsIntelligence(homeName, awayName) {
  ...
  try {
    ...
    const { payload } = await cachedProviderCall({
      cacheKey: `newsapi:intelligence:v7:${pair}`,
      provider: 'NewsAPI',
      ttlSeconds: 900,
      staleSeconds: 21600,
      loader: async () => {
...
module.exports = { getNewsIntelligence, classify, mentions, footballContext };
