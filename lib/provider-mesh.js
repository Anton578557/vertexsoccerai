...
const PROVIDERS = Object.freeze({
  ...
  sportmonks: {
    ...
    roles: ['fixtures', 'results', 'recent_form', 'statistics', 'events', 'lineups', 'players'],
    priority: 65
  },
  wikidata: {
    label: 'Wikidata', tier: 'open', auth: 'none', roles: ['team_aliases', 'team_identity'], priority: 55,
    licenseNote: 'CC0 structured data. Club identity only, never match probabilities.'
  },
  espn: {
    label: 'ESPN', tier: 'public_fallback', auth: 'none', roles: ['fixtures', 'results', 'recent_form', 'logos'], priority: 60,
    licenseNote: 'Public website feed; no contracted API or availability guarantee. Attribution retained; Disabled by default following a server access denial; enable explicitly only after provider access is established.'
  },
  footballDataCoUk: {
  ...
  openLigaDb: {
    ...
    tier: 'open',
    auth: 'none',
    roles: ['historical_results', 'recent_form', 'league_fallback'],
    priority: 50,
  ...
  bsd: {
    label: 'BSD', tier: 'free_account', auth: 'BSD_API_KEY',
    roles: ['historical_results', 'recent_form'], priority: 44,
    licenseNote: 'Free football REST allowance; owner account and live coverage verification required. No paid add-ons.'
  },
  statsBombOpen: {
...
function isConfigured(key) {
  switch (key) {
    ...
    case 'wikidata':
      return true;
    case 'espn':
      return process.env.ESPN_FOOTBALL_ENABLED === 'true' && process.env.ESPN_FOOTBALL_DISABLED !== 'true';
    case 'bsd':
...
function getPublicProviderStatus() {
  return Object.fromEntries(Object.entries(PROVIDERS).map(([key, provider]) => [key, {
    ...
    activeInAnalysis: ['footballData','apiFootball','theSportsDb','footballDataCoUk','openWeather','newsApi','sportmonks','wikidata','openFootball','openLigaDb'].includes(key) || (key === 'espn' && isConfigured(key)) || (key === 'bsd' && isConfigured(key) && process.env.BSD_FOOTBALL_ENABLED === 'true'),
