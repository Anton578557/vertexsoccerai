...
const PROVIDERS = Object.freeze({
  ...
  espn: {
    ...
    licenseNote: 'Public website feed; no contracted API or availability guarantee. Attribution retained; can be disabled with ESPN_FOOTBALL_DISABLED.'
  },
  footballDataCoUk: {
...
function isConfigured(key) {
  switch (key) {
    ...
    case 'wikidata':
      return true;
    case 'espn':
      return process.env.ESPN_FOOTBALL_DISABLED !== 'true';
    case 'openWeather':
...
function getPublicProviderStatus() {
  return Object.fromEntries(Object.entries(PROVIDERS).map(([key, provider]) => [key, {
    ...
    activeInAnalysis: ['footballData','apiFootball','theSportsDb','footballDataCoUk','openWeather','newsApi','sportmonks','wikidata','espn'].includes(key),
