...
function nameMentioned(text, playerName) {
  ...
  return Boolean(surname && haystack.split(' ').includes(surname));
}
...
async function enrichAvailabilityIntelligence(analysis) {
  ...
  const currentHome = Number(analysis?.newsImpact?.homePct || 0);
  const currentAway = Number(analysis?.newsImpact?.awayPct || 0);
  const homeCombined = Math.round(clamp(currentHome + availability.home.incrementalAttackPct, -14, 9));
  const awayCombined = Math.round(clamp(currentAway + availability.away.incrementalAttackPct, -14, 9));

  analysis.newsImpact = {
    ...(analysis.newsImpact || {}),
    homePct: homeCombined,
    awayPct: awayCombined,
    usedSignals: Number(analysis?.newsImpact?.usedSignals || 0) + availability.usedSignals,
    keyPlayerSignalsUsed: availability.usedSignals,
    appliedToModel: Boolean(availability.usedSignals || analysis?.newsImpact?.appliedToModel)
  };
