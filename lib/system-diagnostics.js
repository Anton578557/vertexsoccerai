...
async function runSystemDiagnostics() {
  ...
  sportmonks.activeInAnalysis = false; // Used by live/upcoming pages, not the prediction pipeline.
  ...
  const coreOk = (workingFootballProviders.length >= 1 || footballDataCoUk.ok) && Boolean(theSportsDb?.ok) && Boolean(supabase?.ok);
