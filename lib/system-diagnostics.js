...
async function checkSupabase() {
  ...
  const [authProbe, serverProbe] = await Promise.all([
    ...
    secret
      ? fetchProbe(`${url}/rest/v1/provider_cache?select=cache_key&limit=1`, { headers: { apikey: secret, Authorization: `Bearer ${secret}`, Accept: 'application/json' } })
...
async function checkResend() {
  ...
  // Production uses a sending-scoped Resend key. Such a key is intentionally
  // not allowed to list domains, so active /domains probing would produce a
  // false 403 even though sending works. Domain verification is managed in
  // Resend; runtime diagnostics verify the required sending configuration.
  return {
