...
async function runSystemDiagnostics() {
  const [apiFootball, sportmonks, rapidApi, footballData, theSportsDb, openWeather, newsApi, supabase, resend, footballDataCoUk, openLigaDb, openMeteo, espn, wikidata] = await Promise.all([
    checkApiFootball(),
    checkSportmonks(),
    checkRapidApi(),
    checkFootballData(),
  ...
  openLigaDb.activeInAnalysis = false;
  ...
  const providers = { apiFootball, sportmonks, rapidApi, footballData, theSportsDb, openWeather, newsApi, supabase, resend, footballDataCoUk, openLigaDb, openMeteo, espn, wikidata };
  ...
  const footballKeys = ['apiFootball', 'sportmonks', 'rapidApi', 'footballData', 'espn'];
  ...
  return {
    ...
    providers,
    staticSources: {
      openFootball: { configured: false, activeInAnalysis: false, mode: 'registered only' },
