...
const MODEL_VERSION = 'Vertex Model 2.0';
...
function scoreDistribution(homeLambda, awayLambda, rho = -0.06, maxGoals = 8) {
  const cells = [];
  let total = 0;
...
function granularPressure(granular) {
  if (!granular?.ok) return { shift: 0, available: false, reason: null };
...
function qualityScore(analysis, home, away, league, pressure, h2h) {
  ...
  const sourceStatus = analysis?.sourceStatus || {};
  const newsReviewed = Boolean(sourceStatus.news && !/unavailable|not connected/i.test(String(sourceStatus.news)));
  ...
  const breakdown = {
    ...
    opponentStrength: analysis?.advanced?.home && analysis?.advanced?.away ? 10 : 0,
    venueSplits: venueSplits ? 10 : 0,
    fixtureResolved: analysis?.fixture?.date ? 8 : 0,
    newsReviewed: newsReviewed ? 7 : 0,
    weather: analysis?.weather ? 4 : 0,
    granularStats: pressure.available || analysis?.granularModel?.ok ? 10 : 0,
...
function buildVertexModelV2(analysis) {
  ...
  if ((homeForm.played || 0) < 3 || (awayForm.played || 0) < 3 || homeForm.avgFor == null || awayForm.avgFor == null) {
    return {
      model: null,
      dataQuality: 0,
      confidence: null,
  ...
  if (Math.abs(formTilt) >= 0.008) {
    pushDriver(drivers, {
      ...
      reason: `weighted PPG ${round(home.ppg, 2)} vs ${round(away.ppg, 2)}`, source: 'Football-Data'
    });
  }
  ...
  if (Math.abs(scheduleTilt) >= 0.006) {
    pushDriver(drivers, {
      ...
      reason: `opponent PPG ${round(home.opponentStrength, 2)} vs ${round(away.opponentStrength, 2)}`, source: 'Football-Data'
    });
  }

  const homeFatigue = fatigueAdjustment(home);
  const awayFatigue = fatigueAdjustment(away);
  homeLambda *= 1 + homeFatigue.adjustment;
  awayLambda *= 1 + awayFatigue.adjustment;
  if (homeFatigue.adjustment) pushDriver(drivers, { key: 'home-fatigue', label: 'Home recovery / congestion', side: 'away', magnitude: homeFatigue.adjustment, reason: homeFatigue.reasons.join(', '), source: 'Football-Data' });
  if (awayFatigue.adjustment) pushDriver(drivers, { key: 'away-fatigue', label: 'Away recovery / congestion', side: 'home', magnitude: awayFatigue.adjustment, reason: awayFatigue.reasons.join(', '), source: 'Football-Data' });

  const newsHome = clamp(finite(analysis?.newsImpact?.homePct, 0) / 100, -0.08, 0.08);
  const newsAway = clamp(finite(analysis?.newsImpact?.awayPct, 0) / 100, -0.08, 0.08);
  ...
  if (h2h.available && Math.abs(h2h.shift) >= 0.004) {
    ...
    pushDriver(drivers, { key: 'h2h', label: 'Recent head-to-head', side: h2h.shift > 0 ? 'home' : 'away', magnitude: h2h.shift, reason: `${analysis.h2h.sample} recent meetings; capped low-weight signal`, source: 'Football-Data' });
  }
  ...
  return {
    ...
    meta: {
      ...
      coverage: {
        ...
        opponentAdjustedForm: Boolean(analysis?.advanced?.home && analysis?.advanced?.away),
        ...
        news: Boolean(analysis?.sourceStatus?.news),
        structuredInjuriesAndLineups: quality.structuredSquad,
        weather: Boolean(analysis?.weather),
        granularEventStats: Boolean(analysis?.granularModel?.ok),
        h2h: h2h.available
      },
      teamPower: {
...
function finalizeVertexModelV2(analysis) {
  ...
  analysis.marketCoverage = {
    ...(analysis.marketCoverage || {}),
    core: ['1X2', 'Double Chance', 'Draw No Bet', 'Goals', 'BTTS', 'Team Goals', 'Clean Sheet']
  };

  const limitations = Array.isArray(analysis.limitations) ? analysis.limitations.filter((item) => !/at least three completed matches/i.test(String(item))) : [];
  if (!result.model) limitations.push('At least three verified completed matches per team are required before Vertex Model 2.0 calculates probabilities.');
