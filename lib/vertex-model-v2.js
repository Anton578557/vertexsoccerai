...
const MODEL_VERSION = 'Vertex Model 2.5';
...
function teamState(form = {}, advanced = {}, venue = 'overall', leagueTeamGoals = 1.3) {
  ...
  const venueBlend = venue !== 'neutral' && venueSample >= 3 ? Math.min(0.58, 0.25 + venueRel * 0.33) : 0;
...
function qualityScore(analysis, home, away, league, pressure, h2h) {
  ...
  const structuredSquad = analysis?.matchContext?.lineups?.confirmed === true;
...
function buildVertexModelV2(analysis, parameters = PARAMETERS) {
  ...
  const neutral=analysis.fixture?.neutralGround===true;
  if(neutral)league.homeGoals=league.awayGoals=(league.homeGoals+league.awayGoals)/2;
  const home = teamState(homeForm, analysis?.advanced?.home || {}, neutral?'neutral':'home', league.teamGoalAvg);
  const away = teamState(awayForm, analysis?.advanced?.away || {}, neutral?'neutral':'away', league.teamGoalAvg);
  const drivers = [];
  const formSource = analysis.sourceStatus?.primaryFootball || 'Verified match history';

  const strengthExponent=finite(parameters.strengthExponent,0.5);
  let homeLambda = league.homeGoals * Math.pow(home.attackStrength * away.defenceWeakness,strengthExponent);
  let awayLambda = league.awayGoals * Math.pow(away.attackStrength * home.defenceWeakness,strengthExponent);
  ...
  const distribution = scoreDistribution(homeLambda, awayLambda,finite(parameters.rho,-0.06));
  ...
  return {
    ...
    meta: {
      ...
      coverage: {
        ...
        homeAwaySplits: !neutral && Boolean((analysis?.advanced?.home?.venue?.matches || 0) >= 3 && (analysis?.advanced?.away?.venue?.matches || 0) >= 3),
        leagueBaseline: league.sample >= 12,
        fixture: Boolean(analysis?.fixture?.date),
        news: quality.breakdown.newsReviewed > 0,
        confirmedLineups:quality.structuredSquad,
        structuredInjuriesAndLineups: quality.structuredSquad && analysis.squad?.completeInjuries === true,
      ...
      basis: {source:formSource, league:{name:analysis.fixture?.league || null,sample:league.sample,
        homeGoals:round(league.homeGoals),awayGoals:round(league.awayGoals)}, neutralGround:neutral},
      teamPower: {
...
function finalizeVertexModelV2(analysis) {
  ...
  const limitations = Array.isArray(analysis.limitations) ? analysis.limitations.filter((item) => !/at least three (?:verified )?completed matches|recent-form sample|not enough completed-match data|structured injury and confirmed-lineup|confirmed starting lineups are unavailable|the injury list is incomplete/i.test(String(item))) : [];
  ...
  if (!result.meta.coverage.confirmedLineups) limitations.push('Confirmed starting lineups are unavailable for this fixture.');
  if (analysis.squad?.completeInjuries !== true) limitations.push('The injury list is incomplete; missing records do not mean every player is available.');
