...
const MODEL_VERSION = 'Vertex Model 2.1';
...
function buildVertexModelV2(analysis) {
  ...
  if (homeFatigue.adjustment) pushDriver(drivers, { key: 'home-fatigue', label: 'Home recovery / congestion', side: homeFatigue.adjustment > 0 ? 'home' : 'away', magnitude: homeFatigue.adjustment, reason: homeFatigue.reasons.join(', '), source: 'Football-Data' });
  if (awayFatigue.adjustment) pushDriver(drivers, { key: 'away-fatigue', label: 'Away recovery / congestion', side: awayFatigue.adjustment > 0 ? 'away' : 'home', magnitude: awayFatigue.adjustment, reason: awayFatigue.reasons.join(', '), source: 'Football-Data' });
  ...
  const model = {
    ...
    scoreScenarios: [...distribution].sort((a, b) => b.p - a.p).slice(0, 5).map((cell) => ({ score: `${cell.home}-${cell.away}`, probability: round(cell.p * 100, 1) })),
...
function finalizeVertexModelV2(analysis) {
  ...
  analysis.marketCoverage = {
    ...(analysis.marketCoverage || {}),
    core: result.model ? ['1X2', 'Double Chance', 'Draw No Bet', 'Goals', 'BTTS', 'Team Goals', 'Clean Sheet', 'Score Scenarios'] : []
  };
