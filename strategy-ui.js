...
(() => {
  ...
  const copy = {
    ru: {
      title:'Моя стратегия', subtitle:'Ваш фокус. Ваш темп. Решения с объяснением.', kicker:'ПЕРСОНАЛЬНЫЙ ПЛАН',
      ...
      start:'Создать мой план', guest:'План сохраняется в вашем аккаунте. Можно пользоваться без денежных ставок.',
    ...
    en: {
      title:'My strategy', subtitle:'Your focus. Your pace. Decisions explained.', kicker:'PERSONAL PLAN', hero:'Less noise.', heroAccent:'More insight.', intro:'Choose leagues, markets and your pace. Vertex checks upcoming games, explains the selection and saves forecasts to review after the match.', start:'Create my plan', guest:'Your plan is saved in your account. No monetary betting required.',
      ...
      options:{ '1X2':'Match result 1X2', Beginner:'Beginner', Intermediate:'Intermediate', Advanced:'Advanced', Conservative:'Strict', Balanced:'Balanced', Aggressive:'Expanded', 'Protect bankroll':'Risk control', 'Controlled growth':'Decision discipline', 'Learn disciplined analysis':'Learn match analysis', 'Double Chance':'Double chance', Goals:'Goals 2.5', BTTS:'Both teams to score', Corners:'Corners', Cards:'Cards' }, picks:{HOME:'Home win',DRAW:'Draw',AWAY:'Away win',OVER:'Over 2.5 goals',UNDER:'Under 2.5 goals',YES:'Both teams score — yes',NO:'Both teams score — no','1X':'Home or draw',X2:'Away or draw','12':'No draw'}, initial:'Your journal uses the first saved pre-match forecast. Probabilities may differ in a later analysis.', counted:'settled observations', noLearning:'One match result does not prove a decision was wrong.'
    },
    es: {
      title:'Mi estrategia', subtitle:'Tu enfoque. Tu ritmo. Decisiones explicadas.', kicker:'PLAN PERSONAL', hero:'Menos ruido.', heroAccent:'Más criterio.', intro:'Elige ligas, mercados y tu ritmo. Vertex revisa los próximos partidos, explica la selección y guarda los pronósticos para comprobarlos al final.', start:'Crear mi plan', guest:'Tu plan se guarda en tu cuenta. No es necesario apostar dinero.', steps:[['Elige tu enfoque','Hasta dos ligas y tus mercados preferidos.'],['Entiende la decisión','Encaja, observar o pasar, con motivos concretos.'],['Revisa el resultado','El pronóstico original se conserva. Comprobación automática de resultados.']],
  ...
  const leagueLabel = v => {const parts=String(v).split('|');return parts.length===2?[window.VertexLocaleContent?.country(parts[0],lang())||parts[0],parts[1]].filter(Boolean).join(' · '):v;};
  ...
  function render() {
    ...
    if(!owner || (loaded&&!p&&panel!=='settings')) {
      el.innerHTML=`<div class="vs-hero"><div><span class="vs-tag">${t.kicker}</span><h3>${t.hero}<span>${t.heroAccent}</span></h3><p>${t.intro}</p><div class="vs-actions">${button('setup',t.start,true)}</div><p class="vs-mini" style="margin-top:14px">${t.guest}</p></div><div class="vs-steps">${t.steps.map((s,i)=>`<div class="vs-step"><b>0${i+1}</b><div><h4>${s[0]}</h4><p>${s[1]}</p></div></div>`).join('')}</div></div>`;
    } else if(!loaded) el.innerHTML=`<div class="vs-empty">${error?esc(message(error)):t.loading}${error?`<div class="vs-actions">${button('retry',t.retry)}</div>`:''}</div>`;
    else {
      el.innerHTML=`<div class="vs-toolbar"><div class="vs-tabs" role="tablist" aria-label="${esc(t.title)}">${['week','journal','settings'].map((v,i)=>`<button type="button" role="tab" data-vs="tab" data-panel="${v}" aria-selected="${panel===v}">${t.tabs[i]}</button>`).join('')}</div><span class="vs-tag">${t.kicker}</span></div>${panel==='settings'?formHTML():panel==='journal'?journalHTML():weekHTML()}${notice?`<p class="vs-notice" role="status">${esc(t[notice] || notice)}</p>`:''}${error?`<p class="vs-notice error" role="alert">${esc(message(error))} ${button('retry',t.retry)}</p>`:''}`;
    }
  }
  function weekHTML() {
  ...
  async function load(silent=false) {
    ...
    pendingLoad=(async()=>{
      ...
      catch(e){ if(expected===epoch&&!silent){error=e;render();} }
    ...
    try {
      ...
      while(cursor<end && running && expected===epoch && thisScan===scanEpoch) {
        ...
        try {
          ...
          if(e.status===429||e.status===401){error=e;break;}
          cards.push({fixture,error:e});
        }
        cursor++;
      }
      if(expected===epoch&&thisScan===scanEpoch)lastScan=Date.now();
    } catch(e){if(expected===epoch&&thisScan===scanEpoch)error=e;}
  ...
  document.addEventListener('click',async event=>{
    ...
    if(action==='track'){
      ...
      try {const data=await request('/api/strategy',{method:'POST',body:JSON.stringify({action:'track',fixtureKey:item.key,market:result.pick.market})});if(expected!==epoch)return;accept(data);notice='initial';error='';}
      catch(e){if(expected===epoch)error=e;}finally{if(expected===epoch)render();}
    }
  });
  ...
  document.addEventListener('submit',async event=>{
    ...
    const profile=readForm();if(!profile?.markets.length){draft=profile;error=new Error('CHOOSE_MARKETS');render();return;}
    const expected=epoch;const b=event.target.querySelector('button[type=submit]');b.disabled=true;b.textContent=c().saving;
    try {const data=await request('/api/strategy',{method:'POST',body:JSON.stringify({action:'profile',profile})});if(expected!==epoch)return;accept(data);panel='week';draft=null;notice='saved';error='';lastScan=0;render();scan();}
    catch(e){if(expected===epoch){draft=profile;error=e;render();}}
  });
