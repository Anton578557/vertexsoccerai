...
(() => {
  ...
  function render() {
    ...
    if(!owner || (loaded&&!p&&panel!=='settings')) {
      ...
      el.innerHTML=`<div class="vs-toolbar"><div class="vs-tabs" role="tablist" aria-label="${esc(t.title)}">${['week','journal','settings'].map((v,i)=>`<button type="button" role="tab" data-vs="tab" data-panel="${v}" aria-selected="${panel===v}">${t.tabs[i]}</button>`).join('')}</div><span class="vs-tag">${t.free}</span></div>${panel==='settings'?formHTML():panel==='journal'?journalHTML():weekHTML()}${notice?`<p class="vs-notice" role="status">${esc(t[notice] || notice)}</p>`:''}${error?`<p class="vs-notice error" role="alert">${esc(message(error))} ${button('retry',t.retry)}</p>`:''}`;
    }
  }
  function weekHTML() {
